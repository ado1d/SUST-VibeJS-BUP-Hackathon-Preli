...
def _factor_from(note: str) -> Optional[float]:
    ...
    if m:
        pct = float(m.group(1))
        if pct > 100.0:
            return None  # solar INCREASE (e.g. "boost to 130%") is unsupported
        return _c(pct / 100.0)
    ...
    for word, frac in sorted(_FRACTIONS.items(), key=lambda kv: -len(kv[0])):
        if re.search(rf"\b{re.escape(word)}\b", low):
            return _c(frac)
    # total loss of solar: "completely unavailable", "no solar at all",
    # "zero output", "down entirely", "produces nothing"
    if re.search(
        r"(?:completely|entirely|fully|totally)?\s*(?:unavailable|offline|out of service)"
        r"|no solar at all|zero (?:solar|output|generation|production)"
        r"|produces? (?:nothing|no output|no power|zero)|not produce \w+"
        r"|(?:solar|generation|output) (?:is |will be |goes |going )?(?:down|off|dark)",
        low,
    ):
        return 0.0
    return None
...
def _pct_or_fraction_capacity(note: str, capacity: float) -> Optional[float]:
    ...
    low = note.lower()
    m = re.search(
        r"(\d+(?:\.\d+)?)\s*%\s*(?:of\s+)?(?:the\s+)?(?:full\s+)?(?:battery\s+)?capacity", low)
    if m:
        return float(m.group(1)) / 100.0 * capacity
    m = re.search(r"(\d+(?:\.\d+)?)\s*%\s+of\s+(?:the\s+)?(?:full\s+)?battery", low)
    if m:
        return float(m.group(1)) / 100.0 * capacity
...
# phrasings that ban grid import entirely -> cap 0
_RE_GRID_BAN = re.compile(
    r"\bno\s+(?:grid\s+)?(?:import|imports|intake|purchase|purchases|draw|draws|drawing)\b"
    r"|\bwithout\s+(?:any\s+)?grid\b"
    r"|\bzero\s+grid\b"
    r"|\bnot\s+(?:use|using)\s+(?:any\s+|the\s+)?grid\b"
    r"|\bgrid\s+(?:is\s+|will\s+be\s+)?(?:off|unavailable|down|banned)\b"
)

# one note implying BOTH a charge and a discharge prohibition
_RE_DUAL_BATTERY_BAN = re.compile(
    r"charg\w+[^.;]{0,25}?(?:and|&|\+)[^.;]{0,25}?discharg\w+"
    r"|discharg\w+[^.;]{0,25}?(?:and|&|\+)[^.;]{0,25}?charg\w+"
)

# strict at-least phrasing that lets a reserve note work without the literal
# word "battery" (e.g. "Keep at least 90 kWh from 11 PM until midnight")
_RE_KEEP_AT_LEAST = re.compile(
    r"\b(?:keep|maintain|retain|hold|reserve)\b[^.;]{0,25}?\bat least\b"
)
...
def classify(note: str, battery_capacity: float, note_index: int) -> dict:
    ...
    # 0) one note implying BOTH charge and discharge prohibitions cannot map
    #    to exactly one directive type -> conservative no_op (spec: one per note)
    if _RE_DUAL_BATTERY_BAN.search(low) and _RE_NO_KW.search(low):
        return result("no_op", None, "Emergency fallback: note implies multiple directives; not applied.")
    ...
    # 2) battery reserve floor (battery + at-least language, or a strict
    #    "keep at least X kWh" phrasing even without the word "battery")
    reserve_lang = (
        _RE_BATTERY_KW.search(low) and _RE_RESERVE_KW.search(low)
    ) or (
        _RE_KEEP_AT_LEAST.search(low)
        and (_kwh_value(note) is not None
             or _pct_or_fraction_capacity(note, battery_capacity) is not None)
        and not _RE_GRID_KW.search(low)
    )
    if reserve_lang:
        if hours is None:
    ...
    # 5) grid import cap (a full import ban is a cap of 0 kWh)
    if _RE_GRID_KW.search(low) and _RE_GRID_LIMIT_KW.search(low):
        kwh = _kwh_value(note)
        if kwh is None and _RE_GRID_BAN.search(low):
            kwh = 0.0
